...
void Player::update_player() {
    ...
    if (EnemyManager::get_instance().is_colliding_with_enemies(get_instance().get_player_pos())) {
        // ...check if their velocity is downwards...
        if (player_y_velocity > 0) {
            // ...if yes, award the player and kill the enemy
            EnemyManager::get_instance().remove_colliding_enemy(get_instance().get_player_pos());
