...
void Player::move_player_horizontally(float delta) {
    ...
    if (!LevelManager::get_instance().is_colliding({next_x, Player::get_instance().get_player_pos_y()}, WALL)) {
...
void Player::update_player_gravity() {
    ...
    if (LevelManager::get_instance().is_colliding({Player::get_instance().get_player_pos().x, Player::get_instance().get_player_pos().y - 0.1f}, WALL) && player_y_velocity < 0) {
...
void Player::update_player() {
    ...
    if (LevelManager::get_instance().is_colliding(Player::get_instance().get_player_pos(), COIN)) {
        LevelManager::get_instance().get_collider(Player::get_instance().get_player_pos(), COIN) = AIR; // Removes the coin
        increment_player_score();
    }

    if (LevelManager::get_instance().is_colliding(Player::get_instance().get_player_pos(), EXIT)) {
    ...
    if (LevelManager::get_instance().is_colliding(Player::get_instance().get_player_pos(), SPIKE) || Player::get_instance().get_player_pos().y > LevelManager::get_instance().get_current_level().get_rows()) {
        kill_player();
    }
